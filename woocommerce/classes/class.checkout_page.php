...
class Stiles_Checkout_Page
{
    ...
    {
        $this->fields_billing     = get_option( 'stiles_wc_fields_billing', array() );
        $this->fields_shipping    = get_option( 'stiles_wc_fields_shipping', array() );
        $this->fields_additional  = get_option( 'stiles_wc_fields_additional', array() );
    ...
    public function country_locale( $fields ) 
    {
        if( is_array( $fields ) )
        {
            if( empty( get_option( 'stiles_wc_fields_billing' ) ) )
            {
               $fieldgroup = 'shipping'; 
            }else{
                $fieldgroup = 'billing';
            ...
            $address_fields = get_option( 'stiles_wc_fields_' . $fieldgroup );
    ...
    {
        if( is_array( $this->fields_additional ) && ! empty( get_option( 'stiles_wc_fields_additional' ) ) )
        {
            $enabled = 0;
    ...
    public function create_fields( $ofields, $default_fields ) 
    {
        if( is_array( $ofields ) && !empty( $ofields ) ) 
        {
            foreach( $ofields as $name => $field ) 
            {
                ...
                $new_field['class'][] = 'stiles-field-area';
                $new_field['class'][] = 'stiles-field-' . $field_type;
                ...
                if( $field_type === 'select' || $field_type === 'radio' )
                {
                    if( isset( $new_field['options'] ) )
                    {
                        ...
                        foreach( $options_arr as $key => $value ) {
                            ...
                            $options[$key] = esc_html__( $value, smw_slug );
                        }
                ...
                {
                    $new_field['label'] = esc_html__( $new_field['label'], smw_slug );
                }

                if( isset( $new_field['placeholder'] ) )
                {
                    $new_field['placeholder'] = esc_html__( $new_field['placeholder'], smw_slug );
                }
    ...
    */
            ...
            {                 
                foreach( $validators as $validator ){
                    ...
                    if( $validator === 'number' )
                    {
                        if( !is_numeric( $value ) )
                        {
                            $err_message = '<strong>'. $flabel .'</strong> '. esc_html__('is not a valid number.',smw_slug);    
                        }
                    }
    ...
    {
        $fields = get_option( 'stiles_wc_fields_'. $key, array() );
            ...
            } else if( $key === 'additional' )
            {
                $fields = array(
                    'order_comments' => array(
                        'type'        => 'textarea',
                        'class'       => array('notes'),
                        'label'       => __('Order Notes', smw_slug),
                        'placeholder' => _x('Notes about your order, e.g. special notes for delivery.', 'placeholder', smw_slug)
                    )
                );
            }
    ...
    public function order_details_after_order_table( $order )
    {
        ...
        if( is_array( $fields ) && !empty( $fields ) )
        {
            ...
            {     
                if( $this->is_custom_field( $field ) && isset( $field['show_in_order'] ) && $field['show_in_order'] )
                {
                    ...
                    if( $value )
                    {
                        ...
                        if( is_account_page() )
                        {
                            if( apply_filters( 'stiles_view_order_customer_details_table_view', true ) ){
                                $output_data .= '<tr><th>'. $label .':</th><td>'. $value .'</td></tr>';
                            }else{
                                $output_data .= '<br/><dt>'. $label .':</dt><dd>'. $value .'</dd>';
                            }
                        }else{
                            if( apply_filters( 'stiles_thankyou_customer_details_table_view', true )){
                                $output_data .= '<tr><th>'. $label .':</th><td>'. $value .'</td></tr>';
                            }else{
                                $output_data .= '<br/><dt>'. $label .':</dt><dd>'. $value .'</dd>';
                            }
            ...
            if( $output_data )
            {
                do_action( 'stiles_order_details_before_custom_fields_table', $order ); 
                ?>
                    <table class="woocommerce-table woocommerce-table--custom-fields shop_table custom-fields">
                        <?php echo $output_data; ?>
                    </table>
                <?php
                do_action( 'stiles_order_details_after_custom_fields_table', $order ); 
            }
        }
    }
}

Stiles_Checkout_Page::instance();