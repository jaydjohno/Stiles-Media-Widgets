...
class Stiles_Template_Library
{
    const TRANSIENT_KEY = 'stiles_template_info';
    public static $buylink = null;

    public static $endpoint = smw_dir . 'includes/admin/json/layoutinfofree.json';
    ...
    public function admin_menu() 
    {
        add_submenu_page(
            'smw_page', 
            esc_html__( 'Stiles Templates', smw_slug ),
            esc_html__( 'Stiles Templates', smw_slug ), 
            'manage_options', 
            'stiles_templates', 
    ...
    {
        require_once smw_dir . 'inc/admin/include/templates_list.php';
    }
    ...
    public function templates_ajax_request(){
        ...
        if ( isset( $_REQUEST ) ) {
            ...
            echo json_encode(
                array( 
                    'id'      => $new_post_id,
                    'edittxt' => !empty( $page_title ) ? esc_html__( 'Edit Page', smw_slug ) : esc_html__( 'Edit Template', smw_slug )
                )
            );
        }
        ...
        foreach ( $plugins as $key => $plugin ) {
            ...
            if ( ! is_wp_error( $data ) ) {
                ...
                if ( file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) && is_plugin_inactive( $data['location'] ) ) {
                    ...
                    $button_text    = esc_html__( 'Activate', smw_slug );

                // Not Installed.
                } elseif ( ! file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) ) {

                    $button_classes = 'button install-now';
                    $button_text    = esc_html__( 'Install Now', smw_slug );

                // Active.
                } else {
                    $button_classes = 'button disabled';
                    $button_text    = esc_html__( 'Activated', smw_slug );
                }
                    ...
                    <li class="htwptemplata-plugin-<?php echo $data['slug']; ?>">
                        ...
                        <?php
                            if ( $type == 'pro' && ! file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) ) {
                                echo '<a class="button" href="'.esc_url( $data['pllink'] ).'" target="_blank">'.esc_html__( 'Buy Now', smw_slug ).'</a>';
                            }else{
                        ?>
                    ...
                    <li class="htwptemplata-theme-<?php echo $data['slug']; ?>">
                        ...
                        <?php
                            if ( !empty( $data['prolink'] ) ) {
                                echo '<a class="button" href="'.esc_url( $data['prolink'] ).'" target="_blank">'.esc_html__( 'Buy Now', smw_slug ).'</a>';
                            }else{
                        ?>
    ...
    public function ajax_plugin_activation() {
        ...
        if ( ! current_user_can( 'install_plugins' ) || ! isset( $_POST['location'] ) || ! $_POST['location'] ) {
            wp_send_json_error(
                array(
                    'success' => false,
                    'message' => esc_html__( 'Plugin Not Found', smw_slug ),
                )
            );
        }
        ...
        wp_send_json_success(
            array(
                'success' => true,
                'message' => esc_html__( 'Plugin Successfully Activated', smw_slug ),
            )
        );
    ...
    public function ajax_theme_activation() {
        ...
        if ( ! current_user_can( 'install_themes' ) || ! isset( $_POST['themeslug'] ) || ! $_POST['themeslug'] ) {
            wp_send_json_error(
                array(
                    'success' => false,
                    'message' => esc_html__( 'Sorry, you are not allowed to install themes on this site.', smw_slug ),
                )
            );
        }
        ...
        wp_send_json_success(
            array(
                'success' => true,
                'message' => __( 'Theme Activated', smw_slug ),
            )
        );
    }
...
Stiles_Template_Library::instance();